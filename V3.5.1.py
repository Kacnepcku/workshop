...
class Application:
    def __init__(self, root):
        ...
        return   # Остальная инициализация после подключения
        ###! +++ 3.5.1 ^^^

    ###! +++ 3.5.1 vvv --------------------------------------------------------------
    def show_connection_dialog(self):
        # Создаём окно с полем ввода для сканера и статусом подключения.
    ...
    def attempt_connection(self):
        # Подключaемся к sql, при ошибке повторяем через 5 секунд.
        try:
            self.conn = pymysql.connect(
                host=DB_HOST,
                user=DB_USER,
    ...
    ###! +++ 3.5.1 ^^^
        ...
        # Отображаем стандартное изображение при запуске
    ...
    def show_auto_message(self, message, title="Информация", timeout=5000, msg_type='info'):
        ...
        win.after(timeout, lambda: (win.destroy())) 
        ###! --- 3.5.1 ------------------------ ^^^ ", self.entry.focus_set()))"

    # *------------------------------------------------------------------
    # * Стандартное изображение (ROSHAN.png)
    # *------------------------------------------------------------------
    ...
    # *------------------------------------------------------------------
    # * Изображения конструкций
    # *------------------------------------------------------------------
    def update_image(self, zaknum, konid):
        ## Загружает и отображает изображение конструкции.
        ## Если файл не найден или ошибка – показывает стандартное изображение.
        self.current_image_path = None
    ...
    def display_image(self, img):
        ...
        if img is None:
            return
        # Обрезка на 40 пикселей со всех сторон (для конструкций, не для стандартного?)
        # Для стандартного тоже применим обрезку (можно убрать, но пусть будет)
    ...
    def on_scan(self, event):
        ...
        ###! +++ 5.3.1 vvv
        # Если соединение ещё не установлено, только код 12345
        if self.conn is None or self.cursor is None:
            if code == '12345':
                self.on_closing()
            else:
                self.show_auto_message("Нет подключения к БД.",
                                       "Ожидание", 3000, msg_type='error')
                self.entry.focus_set()
            return
        ###! +++ 5.3.1 ^^^
    ...
    # - если gri='3' (импост) – игнорируем
    # - для gr 1-4 – разрешена только своя группа (r или s)
    # - для gr 5-6 – разрешены и рамы, и створки, но проверяем дубли по "bci"
    # ------------------------------------------------------------------
