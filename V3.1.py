...
def get_dimensions_and_perimeter_area_for_bci_gri(cursor, bci, gri):
    # Сначала пробуем взять раму (GRI='r')
    cursor.execute(
        "SELECT ORIENT, LENGTH FROM CCALC WHERE BCI = %s AND GRI = 'r' AND ORIENT IN ('Н', 'П')",
        (bci,)
    )
    rows = cursor.fetchall()
    width_mm = None
    ...
    # Если рамы нет, пробуем створку (GRI='s')
    cursor.execute(
        "SELECT ORIENT, LENGTH FROM CCALC WHERE BCI = %s AND GRI = 's' AND ORIENT IN ('Н', 'П')",
        (bci,)
    )
    rows = cursor.fetchall()
    if rows:
    ...
    # Если и створки нет, берём любую другую группу
    cursor.execute(
        "SELECT ORIENT, LENGTH FROM CCALC WHERE BCI = %s AND ORIENT IN ('Н', 'П')",
        (bci,)
    )
    rows = cursor.fetchall()
    if rows:
...
class Application:
    def __init__(self, root):
        ...
        # Привязываем события для возврата фокуса на поле ввода
        ...
        # Определяем разрешённые группы для данного рабочего места по GR
        if self.gr in (1, 2):
            self.allowed_gri = ['r']
        elif self.gr in (3, 4):
            self.allowed_gri = ['s']
        ...
        # ------------------- Интерфейс -------------------
        # Создаём PanedWindow для разделения на левую и правую части
        # Увеличиваем вес правой части для большей картинки
        self.paned = ttk.PanedWindow(root, orient=tk.HORIZONTAL)
        self.paned.pack(fill=tk.BOTH, expand=True)

        # Левая часть (текстовая информация) – weight=1
        self.left_frame = ttk.Frame(self.paned)
        self.paned.add(self.left_frame, weight=1)

        # Правая часть (графическая) – weight=3 (в 3 раза шире левой)
        self.right_frame = ttk.Frame(self.paned)
        self.paned.add(self.right_frame, weight=3)

        # --- Содержимое левой части ---
        # Верхняя панель: название места и информация о занятости
        ...
        self.info_line3 = tk.Label(self.detal_frame, text="", font=("Arial", 20, "bold"), fg="darkblue", anchor='w')
        ...
        # Блок статистики за смену (шрифт увеличен на 25% – с 16 до 20)
        ...
        self.tree_workers = ttk.Treeview(
            ...
            show="headings",
            height=6  # уменьшено с 10 до 6 строк
        )
        # Заголовки с короткими названиями
        ...
        # Устанавливаем компактные ширины
        ...
        # Нижняя панель: поле ввода (кнопка выхода удалена)
        ...
        # --- Содержимое правой части (графика) ---
        # Используем цвет фона, совпадающий с основным окном
        bg_color = self.root.cget('bg')
        self.image_label = tk.Label(self.right_frame, bg=bg_color)
        self.image_label.pack(expand=True, fill=tk.BOTH)

        # При изменении размеров правого фрейма пересчитываем картинку
        ...
        # Храним текущее изображение для возможности пересчёта
        self.current_image_path = None

    # ------------------------------------------------------------------
    # Обработчики событий для возврата фокуса на поле ввода
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Всплывающее окно без кнопок с заданным стилем, увеличенное и центрированное
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Обновление изображения в правой части (путь обновлён)
    # ------------------------------------------------------------------
    def update_image(self, zaknum, konid):
        """
        Загружает и отображает изображение в правом фрейме.
        Имя файла: [ZAKNUM] ([KONID]).jpg
       
        Обрезка на 40 пикселей со всех сторон.
        При отсутствии файла – очищает область.
        """
        self.current_image_path = None
    ...
    def display_image(self, img):
        ...
        # Получаем размеры правого фрейма
        fw = self.right_frame.winfo_width()
        fh = self.right_frame.winfo_height()
        if fw < 20 or fh < 20:
            fw = 400
            fh = 300
        # Минимальные отступы для увеличения изображения
        padding = 5
    ...
    def on_right_frame_resize(self, event):
        """Обработчик изменения размера правого фрейма – перерисовывает изображение."""
        ...
        try:
            ...
            # Не выводим ошибку, просто игнорируем
            pass

    # ------------------------------------------------------------------
    # Закрытие всех сессий сотрудников на текущем рабочем месте
    # ------------------------------------------------------------------
    def close_all_worker_sessions(self):
        """Закрывает все открытые сессии сотрудников на текущем рабочем месте."""
        try:
            self.cursor.execute(
    ...
    # ------------------------------------------------------------------
    # Очистка информационного блока детали
    # ------------------------------------------------------------------
    ...
    def set_detal_info(self, detal_info, width_mm, height_mm, perimeter_m, area_m2):
    ...
    # ------------------------------------------------------------------
    # Закрытие предыдущих сессий сотрудников
    # ------------------------------------------------------------------
    ...
    def on_scan(self, event):
        ...
        if not code:
            self.entry.focus_set()
            return
        # Код 12345 – выход из программы с закрытием всех сессий
        if code == '12345':
    ...
    # ------------------------------------------------------------------
    # Обработка детали (основная логика)
    # ------------------------------------------------------------------
    def process_detal(self, detal):
        try:
            ...
            if gri not in self.allowed_gri:
                group_names = {'r': 'РАМЫ', 's': 'СТВОРКИ'}
            ...
            if open_session:
                # Деталь уже зарегистрирована на этом месте – только предупреждение
                msg = (f"Деталь {detal} (ZAKNUM:{detal_info['zaknum']}) уже зарегистрирована на {self.workplace_name}.")
                log_message(f"ПРЕДУПРЕЖДЕНИЕ: {msg}")
                self.show_auto_message(msg, "Предупреждение", 5000, msg_type='error')
                self.conn.rollback()
                # Всё равно показываем информацию
            ...
            # Успешная регистрация – показываем зелёное окно на 3 секунды
